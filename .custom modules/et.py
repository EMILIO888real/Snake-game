...
def clear_lines(lines_to_clear:int = 1, force_clear:bool = True) -> None:
    ...
    Parameters:
        lines_to_clear: specify the count of lines to clear
        force_clear: specify whether to truly clear specified lines or just moved the cursor back.
    '''
    text_clearer = '\033[F'
    if force_clear:
        text_clearer += '\033[2K'
...
def read_json(file_name: str | Path, default_values: Optional[dict] = None, indent: int = 4) -> dict:
    '''
...
def copy_with_exceptions(source: str | Path, destination: str | Path, exceptions: Sequence[str]):
    ...
    for item in listdir(source):
        if item in exceptions:
            continue
        item = Path(source + '/' + item)
        if Path(item).is_file():
            copy2(item, Path('./archive'))
        else:
            copytree(item, Path('./archive/' + str(item)))
